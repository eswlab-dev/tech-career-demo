...
export async function executeAction(req, res) {
  ...
  console.log(`executeAction -> req.body`, req.body);
  ...
  try {
    ...
    console.log(`executeAction -> text`, text);
    if (!text) {
    ...
    console.log(`executeAction -> transformedText`, transformedText);

    const isChanged = await mondayService.changeColumnValue(
      shortLivedToken,
      boardId,
      itemId,
      targetColumnId,
    ...
    console.log(`executeAction -> isChanged`, isChanged);
    if (isChanged)
      await mondayService.notify(
        userId,
        itemId,
        transformationType,
        isChanged,
